...
public class Form_Login extends JFrame {
    ...
    public static Client runClient(String name) {
        Client client = null;
    ...
    public void giaovien() {
        if (tfdangnhaptk1.getText().isEmpty() || tfdangnhapmk1.getPassword().length == 0) {
            JOptionPane.showMessageDialog(null, "Vui lòng điền đầy đủ thông tin");
            return;
        }

        String username = tfdangnhaptk1.getText();
        String password = new String(tfdangnhapmk1.getPassword());
        try {
            client = runClient(username);
            if (client != null) {
                client.loginStaff(username, password);
    ...
    public void hocsinh() {
        if (tfdangnhaptk.getText().isEmpty() || tfdangnhapmk.getPassword().length == 0) {
            JOptionPane.showMessageDialog(null, "Vui lòng điền đầy đủ thông tin");
            return;
        }

        String username = tfdangnhaptk.getText();
        String password = new String(tfdangnhapmk.getPassword());
        try {
            client = runClient(username);
            if (client != null) {
    ...
    public Form_Login() {
        ...
        URL url_hhd = Form_Login.class.getResource("/Design/logo.png");
        if (url_hhd != null) {
        ...
        btlogin1.addActionListener(e -> {
            Thread thread = new Thread(this::hocsinh);
            thread.start();
        });
        ...
        btlogin2.addActionListener(e -> {
            Thread thread = new Thread(this::giaovien);
            thread.start();
        });
